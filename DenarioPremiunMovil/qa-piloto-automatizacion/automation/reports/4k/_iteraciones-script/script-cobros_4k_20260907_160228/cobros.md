...
- ✅ **DM-COB-008** Marcar documento → total en Pagos _(marcados: 4 · total: "Monto total a pagar Bs: 635.100,00 Diferencia Bs: -635.100,0")_
...
- ❌ **DM-COB-050** 10% + 80% = 90% > 85% ⇒ se rechaza _(alerta: "Denario Cobros · Se superó el límite de descuento (85%). Máximo disponible: 75%." · «Probando» quedó marcado: true (debe ser false — se QUITA, no se clampea) · «DESC 10 TEST» sigue marcado: true)_
- 🚫 **DM-COB-051** Tasa escrita: 85% acepta · 86% rechaza _(el catálogo no trae ningún descuento de TASA EDITABLE (require_input=true). Crear uno en Empresa → Configuración → Descuentos para Cobros y sincronizar)_
- ✅ **DM-COB-009** Tab Pagos → modal métodos de pago _(métodos: Efectivo, Cheque, Depósito, Transferencia, Pago Móvil)_
- ✅ **DM-COB-040** Completar pago Efectivo = total → diferencia azul _(monto: 63510000 · antes: -635.100,00(red) · después: 0,00(blue))_
- ✅ **DM-COB-012** Diferencia rojo (insuf.) → azul (cubre) _(antes: red · después: blue)_
- ✅ **DM-COB-043** Diferencia se actualiza con el monto _(antes: -635.100,00 · después: 0,00)_
...
- 🚫 **DM-COB-024** Reabrir Guardado → los datos persisten _(no hay nada que cotejar: la foto previa quedó vacía (0 campos leídos, 0 con valor). El cobro se reabrió (tabs: 5), pero eso NO prueba persistencia)_
- ⬜ **DM-COB-019** Enviar cobro _(no se pudo reabrir el cobro guardado)_
...
**Resumen:** N/A:5 · PASS:19 · FAIL:2 · BLOCKED:14
_Tiempo: 94.0s_