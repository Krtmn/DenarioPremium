...
export class CurrencyService {
  ...
  /** Conversión visible para un módulo (currencyModule + multimoneda + show_conversion + tasa). */
  canShowModuleConversion(coModule: string): boolean {
    if (this.globalConfig.get('currencyModule').toLowerCase() !== 'true' || !this.multimoneda) {
      return false;
    }
    const cm = this.getCurrencyModule(coModule);
    if (cm.idModule <= 0) {
      return false;
    }
    return cm.showConversion && this.hasValidExchangeRate();
  }
