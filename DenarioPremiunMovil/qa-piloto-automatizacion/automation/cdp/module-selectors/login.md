...
### Notas por cliente
- ✅ **kron (Isla Coche v1.0/db19): 0 selectores nuevos** — inputs por placeholder (`Usuario`/`Contraseña`, **sin atributo `name`**), submit `ion-button[type="submit"]` o texto que incluya `INICIA`, `ion-checkbox` por `mouse.click` al centro, alert filtrado por `:not(.overlay-hidden)` + `offsetParent!==null`. Todo el estándar consolidado aplicó sin ajustes. ⚠ **Pero el agente LOGIN dejó `__qaH` con solo 2 skills y `__qaDataHook=false`** ⇒ los agentes 2-10 se quedaron sin hook de payload heredado: **la receta de arranque de `_comunes.md` (instalar el bundle completo con `installPayloadCapture`) NO se cumplió.** Verificar `Object.keys(window.__qaH)` + `!!window.__qaDataHook` al cerrar LOGIN. `[kron-20260817]`
- ✅ **run_vzla (La Tortuga v1.0/db19): 6/6 PASS, 0 selectores nuevos** — inputs por placeholder (sin `name`), submit `ion-button[type="submit"]` con `pg.mouse.click` simple, `ion-checkbox` al centro del rect, alert de validación con botón **"OK"** (no "Aceptar"). Sync ~90-120 s → `app-home`. ⚠ **La empresa NO es confirmable por HOME** (por diseño solo se muestra dentro de los formularios) y la **playa declarada no queda confirmada en LOGIN**: se cierra en CLIENTES por el host del POST. 🔴 **2.ª corrida seguida en que LOGIN deja `window.__qaH` VACÍO y `__qaDataHook=false`** (reconfirma `[kron-20260817]`) ⇒ la regla estable para los agentes 2-10 es **comprobar, nunca heredar ni reinstalar a ciegas** (ver `_comunes.md`). `[run_vzla-20260818]`
